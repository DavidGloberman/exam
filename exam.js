...
const Mission1 = (militaryUnit) => {
  let a = militaryUnit.commandStructure.chiefOfStaff;
  return `rank: ${a.rank},  name: ${a.name}, phene: ${a.contact.phone},`;
};
...
const Mission3 = (NewDeployment, militaryUnit) => {
  let currDeployment = militaryUnit.currentDeployment;

  let NewHistory = {
    eventDate: currDeployment.estimatedEndDate,
    eventDescription: `${currDeployment.mission} in ${currDeployment.location}`,
  };
...
const Mission4 = (newFirearm, militaryUnit) => {
  let firearms = militaryUnit.equipment.firearms;
  let a = firearms.find(
    (p) => p.type === newFirearm.type && p.status === newFirearm.status
  );
  if (a) {
    a.quantity += newFirearm.quantity;
    return militaryUnit;
  }
...
const Mission5 = (militaryUnit) => {
  let a = militaryUnit.trainingPrograms.reduce((acc, curr) => {
